...
{
	/** 日志 */
	private static final Logger logger = LoggerFactory.getLogger("lzr.db.ibatis");
	 ...
	 *            事件监听器
	 */
	public IbatisDBServiceImpl(IEventListener<IEvent<?>> eventListenter,File file)
	{
	...
	public IbatisDBServiceImpl(IEventListener<IEvent<?>> eventListener,URL url) 
	{
	...
	@Override
	public <T extends BaseEntity<?>> List<T> getAll(final Class<T> entityClazz) 
		throws DataAccessException {
		throw new UnsupportedOperationException();
	}

	@Override
	public <T> List<T> findByNamedQuery(final String queryName) throws DataAccessException {
		return findByNamedQueryAndNamedParam(queryName, null, null);
	}

	@Override
	public <T> List<T> findByNamedQueryAndNamedParam(final String queryName, final String[] paramNames, final Object[] values)
			throws DataAccessException {
