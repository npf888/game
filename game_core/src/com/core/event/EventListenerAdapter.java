...
{
	private static final Logger logger = LoggerFactory.getLogger("lzr.event");
	private final Map<Class, List<IEventListener>> eventListener = CollectionUtil.buildHashMap();
	...
	public void addListener(Class eventClass, IEventListener listener)
	{
		...
		{
			logger.info("[#GS.EventManager.addListener] [Register event listener (event:"
							+ eventClass.getName()
							+ " listener:"
	...
	{
		List<IEventListener> _eventListeners = this.eventListener.get(event.getClass());
