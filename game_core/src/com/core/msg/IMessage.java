...
/**
 * 消息的接口,源自天书mmo_core中的同名类 消息数据包的格式 <len(2字节)>+<type(2字节)>+<body(len-4)>
 * 
 */
public interface IMessage {
	...
	public static class Packet {
		 ...
		 */
		public static int peekPacketLength(ByteBuffer buf) {
		 ...
		 *             如果从buf中取得到包长度<MIN_MESSAGE_LENGTH或者大于MAX_MESSAGE_LENGTH会抛出此异常
		 */
		public static int popPacketLength(ByteBuffer buf) {
		 ...
		 *             如果数据包中的类型小于0会抛出此异常
		 */
		public static short peekPacketType(ByteBuffer buf) {
		 ...
		 *             如果数据包中的类型小于0会抛出此异常
		 */
		public static short popPacketType(ByteBuffer buf) {
			return seekType(buf, false);
		}

		private static int seekLength(ByteBuffer buf, boolean peek) {
		...
		private static short seekType(ByteBuffer buf, boolean peek) {
			short _type = seekShort(buf, peek);
			return _type;
		}

		private static short seekShort(ByteBuffer buf, boolean peek) {
		 ...
		 * @return
		 */
		protected static int seekIntFromUnsignedShort(ByteBuffer buf,boolean peek) {
			if (buf.remaining() >= HEADER_LEN_BYTES) {
				int _op = buf.position();
				//int _value = buf.getShort() << 16 >>> 16;
				int _value = buf.getUnsignedShort();//AAA 修改于2017年
				if (peek) {
					buf.position(_op);
				}
