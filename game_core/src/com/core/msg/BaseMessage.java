...
/**
 * IMessageBase的基本实现,源自天书mmo_core的同名类
 * 
  *
 */
public abstract class BaseMessage implements IMessage {
	...
	public boolean write() throws MessageParseException {
		try {
			...
			if (messageLength > MAX_MESSAGE_LENGTH / 2) {
				Loggers.msgLogger.warn("[#message too big]"
	...
	public boolean read() throws MessageParseException {
		try {
			...
			type = readShort();
			SharedConstants.uplinkLen=SharedConstants.uplinkLen+this.getLength();
			SharedConstants.uplinkCount=SharedConstants.uplinkCount+1;
			return readImpl();
		} catch (Exception e) {
	...
	protected void writeMessage(IMessage msg) throws MessageParseException {
		ByteBuffer _buf = ByteBuffer.allocate(msg.getInitBufferLength());
	...
	protected void readMessage(IMessage msg) throws MessageParseException {
		...
		ByteBuffer _buf = ByteBuffer.allocate(_length);
	 ...
	 * @return true,写入成功;false,写入失败
	 */
	protected abstract boolean writeImpl();
