...
import com.core.command.ICommand;
import com.core.command.ICommandProcessor;
import com.core.session.ISession;

/**
 * 聊天命令处理器，参考自天书命令管理器代码
 * @author Thinker
 * 
 * 
 */
public class CommandProcessorImpl<T extends ISession> implements ICommandProcessor<T>
{
	...
	public void execute(T sender, String content) 
	{
		...
		{
			return;
		} else if (commands[0].length() <= ICommand.CMD_PREFIX.length())
		{// 命令以"!"开始
			return;
		} else if (commands[0].substring(0, ICommand.CMD_PREFIX.length()).equals(ICommand.CMD_PREFIX))
		{
		...
		try
		{
			...
			if (cmd != null)
			{
				...
				{
					String[] params = Arrays.copyOfRange(commands, 1,commands.length);// 命令参数
					if (logger.isDebugEnabled())
					{
						logger.debug("command " + cmd.getCommandName()+ "'s parameter array : "+ Arrays.toString(params));
					}
