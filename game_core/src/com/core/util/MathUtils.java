...
public class MathUtils
{
    ...
    private static final int BASECHANCE=10000;//概率总数
	...
	public static int randomSelectByFrequency(final int[] frequencies,BitSet excludeIndexSet) 
	{
		...
		for (int i = fromIndex; i < toIndex; i++)
		{
			...
			{
				// 非法频率数据
				throw new IllegalArgumentException("frequency must not be negative. freqencies:"+ Arrays.toString(frequencies));
			}
			total += frequencies[i];
		}
		if (total <= 0)
	 ...
	 * @return 发生的事件索引，即在frequencies中的索引；频率全部为0则返回-1，表示没有事件发生
	 */
	public static int randomSelectByFrequency(Map<Integer, Integer> frequencies,BitSet excludeIndexSet)
	{
		if (frequencies == null) {
			throw new IllegalArgumentException("frequencies is null");
		}
	...
	//	int fromIndex = 0;
	//	int toIndex = frequencies.size();
		int total = 0;
		for (Map.Entry<Integer, Integer> i : frequencies.entrySet()){
			
			if (excludeIndexSet != null && excludeIndexSet.get(i.getKey())) {
				continue;
			}
			if (i.getValue() < 0) {
				// 非法频率数据
	...
	/**
	  * 根据两个比例值抽取其中一个对象
	  * @return 0说明随机到第一个比例，1说明随机到第2个比例 
	  */
	 public static boolean Countchance(int scaleChance){
		float tmpScale=(float)scaleChance/BASECHANCE;
		float randomScale= (float)Math.random()*1;
		if(randomScale<=tmpScale){
			return true;
		}
		return false;
	 }
	 ...
	 public static void main(String[] str){
		 boolean a=MathUtils.Countchance(5000);
