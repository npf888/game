...
public class LRUHashMap<K, V> extends LinkedHashMap<K, V>
{
	...
	/** 淘汰策略 */
	private transient final LRUHashMap.EvictPolicy<V> evictPolicy;
	/** 命中次数 */
	...
	/** 默认的淘汰策略:总是淘汰 */
	private transient final LRUHashMap.EvictPolicy<V> DEFAULT = new LRUHashMap.EvictPolicy<V>()
	{
		@Override
	...
	public LRUHashMap(int maxSize, LRUHashMap.EvictPolicy<V> evictPolicy)
	{
		super(10, 0.75f, true);
		if (maxSize <= 0)
		{
	...
	{
		return "cache size:" + size() + "/" + this.maxSize + ",hit:" + getHitCount() + ",miss:" + getMissCount()
				+ ",hitRate:" + (int) (getHitRate() * 100) + "%";
	}
