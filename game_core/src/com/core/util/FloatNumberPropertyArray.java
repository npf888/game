...
public final class FloatNumberPropertyArray implements Cloneable 
{
	...
	{
		KeyValuePair<Integer, Float>[] changedPairs = new KeyValuePair[bitSet.cardinality()];
		for (int i = bitSet.nextSetBit(0), j = 0; i >= 0; i = bitSet.nextSetBit(i + 1), j++) {
			changedPairs[i] = new KeyValuePair(Integer.valueOf(i), Float.valueOf(this.values[i]));
		}
		return changedPairs;
	}
