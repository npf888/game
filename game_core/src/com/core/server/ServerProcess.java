...
import com.core.msg.filter.DifferenceFilter;
import com.core.msg.filter.FloodByteAttackFilter;
import com.core.msg.filter.FloodCmdAttackFilter;
...
public class ServerProcess {
	...
	/**是否禁用洪水监控:内部服务器不监控**/
	private boolean ifCloseFilter=false;
	 ...
	 * @param ifCloseFilter
	 */
	public void setCloseFilter(boolean ifCloseFilter)
	{
		this.ifCloseFilter=ifCloseFilter;
	}
	/**
	...
	public void start() throws IOException {
		...
		for (int i = 0; i < portList.length; i++) {
			...
			// 添加防洪水攻击过滤器
			FloodByteAttackFilter byteFilter=new FloodByteAttackFilter(this.misIps);
			byteFilter.setCloseFilter(ifCloseFilter);
			cfg.getFilterChain().addFirst("floodCheck_byte", byteFilter);
			
			FloodCmdAttackFilter cmdFilter=new FloodCmdAttackFilter(this.misIps);
			cmdFilter.setCloseFilter(ifCloseFilter);
			cfg.getFilterChain().addFirst("floodCheck_cmd", cmdFilter);

			
			DifferenceFilter diffFilter = new DifferenceFilter(messageRecognizer);
			cfg.getFilterChain().addLast("coderFilter", diffFilter);
//			cfg.getFilterChain().addLast("codec", new ProtocolCodecFilter(new GameCodecFactory(messageRecognizer)));
			int port = portList[i];
			InetSocketAddress _address = this.ip == null ? new InetSocketAddress(port): new InetSocketAddress(ip, port);
