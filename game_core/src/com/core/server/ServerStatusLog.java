...
import com.core.util.ServerVersion;
...
public class ServerStatusLog 
{
	...
	private static final ServerStatusLog instance = new ServerStatusLog(System.getProperty("user.dir")
			+ File.separator + "logs" + File.separator + "server_status");

	
	public ServerStatusLog(String serverStatusLog)
	{
		if (serverStatusLog == null|| (serverStatusLog = serverStatusLog.trim()).length() == 0) 
		{
			throw new IllegalArgumentException("The threadNumberPerPort must not be null ");
		}
	...
	private void writeStatusLog(int status)
	{
		...
		try
		{
			...
			String _time = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(new Date());
	...
	private void checkLogDir() 
	{
		...
		if (_parentFile.exists()) return;

		if (_parentFile.mkdirs()) return;
