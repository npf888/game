...
public class TemplateObjectAssembler {
	...
	public void doAssemble(Object obj, HSSFRow row, Class<?> clazz)
			throws Exception {
	...
	private void insertCollection(Field field, Object obj, HSSFRow row) {
	...
	private Object getArray(Class<?> fieldType, ExcelCollectionMapping ecm,
		...
		for (int i = 0; i < cns.length; i++) {
			...
			if (element_cl.isAnnotationPresent(ExcelRowBinding.class))
				...
				o = this.getFieldValue(null, row, element_cl, new Integer(
						strs[0]));
			}
			Array.set(arr, i, o);
		}
	...
	private void insertSetOrList(Collection<Object> col,
			ExcelCollectionMapping ecm, HSSFRow row) throws Exception {
	 ...
	 * @return
	 */
	private Object getElementObject(Class<?> clazz, String[] strs, HSSFRow row) {
		...
		try {
			...
			for (int j = 0; j < strs.length; j++) {
				...
				fValue = getFieldValue(curField, row, curField.getType(),
						number);
