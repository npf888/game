...
import com.core.annotation.FixUpByCellRange;
...
public class TemplateFileParser {
	...
	protected Map<Integer, TemplateObject> parseXlsSheet(HSSFSheet sheet,
			Class<?> clazz) throws InstantiationException,IllegalAccessException ,Exception
{
		...
		for (int i = 1; i <= Short.MAX_VALUE; i++) {
			...
			{
				throw new Exception("表格中主键ID重复："+sheet.getSheetName()+"|"+obj.getId());
			}
