 ...
 *
 */
public class SlotType25 extends SlotBase{
	...
	public SpecialConnectInfoData specificSlot(int free, int bet,
		...
		ScatterInfoType25 tempScatterInfo = getScatterInfo();
		//处理Scatter
		...
		if(tempScatterInfo.getWild() >0){
			GCSlotType25WildInfo gCSlotType25WildInfo = new GCSlotType25WildInfo();
			gCSlotType25WildInfo.setPosList(ArrayUtils.intList2Array(tempScatterInfo.getWildList()));
			gCSlotType25WildInfo.setWildNum(tempScatterInfo.getWild());
			gCSlotType25WildInfo.setAllPosList(ArrayUtils.intList2Array(tempSlotElementsCope));
			player.sendMessage(gCSlotType25WildInfo);
		}
		//bonus 是否大于 配置中的（3）个
		List<BonusNinjaTemplate>  bonusNinjaTemplates= Globals.getBonusNinjaService().getBonusNinjaTemplateList();
		for(BonusNinjaTemplate bonusNinjaTemplate:bonusNinjaTemplates){
			if(tempScatterInfo.getBnus()>=bonusNinjaTemplate.getBonusNum()){
				GCSlotType25BounsInfo bounsInfo = new GCSlotType25BounsInfo();
				...
				bounsInfo.setPosition(position);
				//把随机位置放到 临时变量里
				HumanTemporaryData humanTemporaryData = player.getHuman().getHumanSlotManager().getHumanTemporaryData();
				humanTemporaryData.setPosition(position);
	 ...
	 * @return
	 */
	protected ScatterInfoType25 getScatterInfo(){
		
		ScatterInfoType25 tempScatterInfo =new ScatterInfoType25();
		//bouns 个数
		int  bounsNum = 0;
		//WILD 个数
		int  wildNum = 0;
        ...
        for(int i = 0;i <  tempSlotElementsCope.size();i++){
        	...
        	if(turnType == SlotElementType.SLOTTYPE25.getIndex()){
        		bounsNum++;
        		...
        		tempScatterInfo.getWildList().add(i);
				++wildNum;
			}
        }
	 ...
	 */
	/*
	protected ScatterInfoType25 getScatterInfo(Human human){
		...
		for(int i=0;i< col;i++){
			...
			for(int j=0;j<row;j++){
				...
				if(tempSlotsTemplate.getSlotElementType() == SlotElementType.SCATTER){
					...
					++tempFoundNum;
				}else if(tempSlotsTemplate.getSlotElementType() == SlotElementType.SLOTTYPE25){
					tempScatterInfo.getBonusList().add(i*row+j);//bouns 的位置
					++bounsNum;
				}else if(tempSlotsTemplate.getSlotElementType() == SlotElementType.bigWILD){
		...
		for(ScatterTemplate tempScatterTemplate :list){
			...
			if(tempFoundNum==scatterNum){
				tempScatterInfo.setScatterTemplate(tempScatterTemplate);
				return tempScatterInfo;
			}else if(scatterNum == 5 && tempFoundNum>=5){
				tempScatterInfo.setScatterTemplate(tempScatterTemplate);
				return tempScatterInfo;
			}
