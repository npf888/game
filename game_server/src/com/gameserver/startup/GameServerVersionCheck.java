...
import com.core.check.BaseVersionCheck;
...
public class GameServerVersionCheck extends BaseVersionCheck<GameServerConfig>
{
	...
	public boolean check()
	{
		...
		do 
		{
			...
			try 
			{
				_resourceVersion = FileUtils.readFileToString(new File(config.getBaseResourceDir()+ File.separator + "version"));
			} catch (IOException e)
			{
			...
			logVersionCheck("resourceVersoin", _resourceVersion, "resourceConfigVersion", config.getResourceVersion(),_match);
			if (!_match) 
			{
				break;
			}
