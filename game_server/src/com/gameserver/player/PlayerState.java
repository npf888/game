...
public enum PlayerState 
{
	...
	static
	{
		...
		loading.setPrevStates(waitingselectrole,creatingrole,loadingrolelist);
		incoming.setPrevStates(loading,leaving);
		entering.setPrevStates(incoming);
		gaming.setPrevStates(entering);// 切换场景失败会进入此状态
		leaving.setPrevStates(gaming);
		logouting.setPrevStates(init,connected,auth,loadingrolelist,waitingselectrole,creatingrole,loading,incoming,entering,gaming,leaving);
		logouted.setPrevStates(init,connected,auth,loadingrolelist,waitingselectrole,creatingrole,loading,incoming,entering,gaming,leaving,logouting);
	}
	...
	{
		if (targetState.prevStates == null) return false;

		for (int i = 0; i < targetState.prevStates.length; i++)
		{
			if (targetState.prevStates[i] == this) return true;
		}
		return false;
	}
}
