...
public class ExportStatusToExcel
{
	...
	public void createExcelSheeet(Map<String, List<StatusInfo>> statusInfos)
	{
		...
		Iterator<Map.Entry<String, List<StatusInfo>>> _statusSet = statusInfos.entrySet().iterator();
		while (_statusSet.hasNext())
		{
			...
			if (_startTime > _tempInfo.getLastUpdateTime())
			{ 
				// 获得最早时间
			...
			if (_endTime < _lastInfo.getLastUpdateTime())
			{ // 获得最晚时间
	 ...
	 * @param statusInfos
	 */
	private void createTableRow(HSSFSheet sheet, short rowIndex,StatusInfo statusInfo)
	{
