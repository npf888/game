...
import com.common.LogReasons.LogDesc;
...
{
	private static final String packageName = "com.logserver.model";
	private static final String reasonParentClass = "com.common.LogReasons";
	...
	public static void main(String[] args) throws Exception 
	{
		...
		{
			final String _className = _file.getName().substring(0,_file.getName().indexOf("."));
			final String _reasonName = reasonParentClass + "$" + _className+ "Reason";
			final Class<?> _class = Class.forName(packageName + "." + _className);
			final Class<?> _reasonClass = Class.forName(_reasonName);
			LogDesc _logReason = (LogDesc) _reasonClass.getAnnotation(LogReasons.LogDesc.class);
			String _logDesc = _logReason.desc();
			if (_logDesc == null || (_logDesc = _logDesc.trim()).length() == 0)
			{
				throw new IllegalStateException("The LogDesc.desc() mest be set.");
			}

			String _tableName = (_className.substring(0, 1).toLowerCase() + _className
					.substring(1)).replaceAll("([A-Z]{1})", "_$1")
					.toLowerCase();
			String _type = invokeMethod(_class.newInstance(), "getType",new Class[0], new Object[0]).toString();
			...
			Enum<?>[] _enums = (Enum<?>[]) invokeStaticMethod(_reasonClass, "values",new Class[0], new Object[0]);
			for (Enum<?> _enum : _enums)
			{
				ReasonDesc _reasonDesc = _enum.getClass().getField(_enum.name()).getAnnotation(ReasonDesc.class);
				SqlReasonDesc.SqlReason _reason = new SqlReasonDesc.SqlReason();
				if (_reasonDesc == null) 
				{
					System.err.println(_enum.name()+ " does not have ReasonDesc Annotation");
					return;
				}
	 ...
	 * 
	 */
	private static List<SqlReasonDesc> sortSqlReason(List<SqlReasonDesc> targetList)
	{
		if (targetList == null)
		{
			return null;
		...
		return Lists.sortedCopy(targetList, new SqlReasonDescSorter());
	}
	 ...
	 *
	 */
	private static class SqlReasonDescSorter implements Comparator<SqlReasonDesc>
	{
		@Override
