...
{
	/** 日志类 */
	private static final String LOG_REASON_CLAZZ = "com.common.LogReasons";
	/** 日志数据字典 */
	...
	private void writeOutGenLogType(GenLogType genLogType, HSSFRow row, RowNumber rownum)
	{
		...
		// 获取日志原因列表并排序
		Collection<GenLogReason> values = this.sortLogReasonList(genLogType._logReasonMap.values());
	 ...
	 * 
	 */
	private Collection<GenLogReason> sortLogReasonList(Collection<GenLogReason> logReasonList) 
	{
		// 创建临时数组列表
	...
	{
		// 获取内置类数组
		Class<?>[] innerClazzes = this.getLogReasonsClazz().getDeclaredClasses();
	...
	private GenLogType createGenLogType(Class<?> enumClazz)
	{
		...
		try 
		{
			...
			for (Object value : values)
			{
				...
				// 添加日志原因到日志类型
				genLogType._logReasonMap.put(genLogReason._id, genLogReason);
			}
		} catch (Exception ex) 
		{
			// 抛出异常
