...
public class GeneratorHelper 
{
	...
	{
		if (msgType.startsWith("com.") && 
			msgType.endsWith(".") == false) 
		{
			// 如果以 com. 开头但是不以 . 结束, 
			// 就认为是一个完整的类名!
			// 那么, 如果已经是一个完整的类名, 
	...
	public static void generate(VelocityContext context,String templateFileName, String outputFile) throws Exception
	{
		//先创建父目录
		FileUtil.mkdir(new File(outputFile).getParent());
		VelocityEngine velocityEngine = new VelocityEngine();
		Properties p = new Properties();
		p.setProperty(Velocity.FILE_RESOURCE_LOADER_PATH, GeneratorHelper.getBuildPath(MessageGenerator.TEMPLATE_DIC));
